...
export class TestingComponent implements OnInit {
  ...
  ngOnInit() {
    this.title = 'Test ' + (parseInt(this.route.snapshot.paramMap.get('id') || '0', 10) + 1);
    this.titleService.setTitle(this.title);
    this.questionService.getQuestionsByQuizId(parseInt(this.route.snapshot.paramMap.get('id') || '0', 10) + 1)
      .pipe(
        finalize(() => {
      ...
      .subscribe((quesitons: Question[]) => this.questions = quesitons);
  }
