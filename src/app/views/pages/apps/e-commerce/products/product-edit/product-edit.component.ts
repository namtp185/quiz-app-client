...
export class ProductEditComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		window.onload = () => {
			const kt_header: HTMLElement | null = document.getElementById('kt_header') || null;
			const style = kt_header ? getComputedStyle(kt_header) : null;
			this.headerMargin = style ? parseInt(style.height, 0) : 0;
		};
	}

	loadProduct(_product: any, fromService: boolean = false) {
		if (!_product) {
			this.goBack('');
		}
	...
	loadProductFromService(productId: any) {
