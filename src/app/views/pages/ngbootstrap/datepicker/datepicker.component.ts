...
const I18N_VALUES: {[name: string]: {weekdays: string[], months: string[]}} = {
	fr: {
		weekdays: ['Lu', 'Ma', 'Me', 'Je', 'Ve', 'Sa', 'Di'],
		months: ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Juin', 'Juil', 'Aou', 'Sep', 'Oct', 'Nov', 'Déc'],
	}
...
export class CustomDatepickerI18n extends NgbDatepickerI18n {
	...
	getWeekdayShortName(weekday: number): string {
    const val = I18N_VALUES[this._i18n.language];
    const result = val?.weekdays[weekday - 1];
    return result ?? '';
	}
	getMonthShortName(month: number): string {
    const val = I18N_VALUES[this._i18n.language];
    const result = val?.months[month - 1];
    return result ?? '';
	}
	getMonthFullName(month: number): string {
		return this.getMonthShortName(month) ?? '';
...
export class DatepickerComponent implements OnInit {
	...
	isTo = (date: any) => equals(date, this.toDate || null);
}
